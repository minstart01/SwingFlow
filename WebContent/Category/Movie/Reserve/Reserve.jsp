...
<script>
	...
	function SearchmName(data) {
		...
		for (var i = 0; i < items.length; i++) {
			$(".lists").append(
					"<li class='sel_local local_off'>" + $(items[i]).text()
							+ "</li>");
		}
		for (var i = 0; i < itemg.length; i++) {
			$(".listg").append(
					"<li class='sel_local local_off' onclick=theaterinfo('g','"
							+ i + "');>" + $(itemg[i]).text() + "</li>");
		}
		for (var i = 0; i < itemp.length; i++) {
			$(".listp").append(
					"<li class='sel_local local_off' onclick=theaterinfo('p','"
							+ i + "');>" + $(itemp[i]).text() + "</li>");
		}
		for (var i = 0; i < itemd.length; i++) {
			$(".listd").append(
					"<li class='sel_local local_off' onclick=theaterinfo('d','"
							+ i + "');>" + $(itemd[i]).text() + "</li>");
		}
		for (var i = 0; i < itemdj.length; i++) {
			$(".listdj").append(
					"<li class='sel_local local_off' onclick=theaterinfo('dj','"
							+ i + "');>" + $(itemdj[i]).text() + "</li>");
		}
		for (var i = 0; i < itemj.length; i++) {
			$(".listj").append(
					"<li class='sel_local local_off' onclick=theaterinfo('j','"
							+ i + "');>" + $(itemj[i]).text() + "</li>");
		}
	...
	function SearchMovie(data) {
		...
		$(".list_movie").append(
				"<li class='sel_movie movie_off'>" + $(names[0]).text()
						+ "<input type='hidden' class='posterimg' value='"
						+ $(names[1]).text() + "'></li>");
	...
	function TdOnclick(a) {
		...
		$(".Td_img").attr("src",
				"/SwingFlow/images/Movie/Reserve/img_PlayDate_on.gif");
	...
	function SearchTime(data) {
		...
		for (var i = 0; i < Time.length; i++) {
			$(".time_list").append(
					"<li class='sel_time time_off'>" + $(Time[i]).text()
							+ "</li>");
		}
		...
		$(".sel_time")
				.click(
						function(e) {
							...
							$(".time_img")
									.attr("src",
											"/SwingFlow/images/Movie/Reserve/img_PlayTime_on.gif");
		...
		function SearchCharge(data) {
			...
			$(".adult_sel_sit").append(
					"<li class='t1 adult_charge'>성인(" + adult + "원)</li>");
			$(".adult_sel_sit").append(
					"<li class='adult_sit_no adult_on'>0</li>");
			for (var i = 1; i < 9; i++) {
				$(".adult_sel_sit").append(
						"<li class='adult_sit_no adult_off'>" + i + "</li>");
			}

			$(".sel_agebox").append("<ul class='sel_sit teen_sel_sit'></ul>");
			$(".teen_sel_sit").append(
					"<li class='t1 teen_charge'>청소년(" + teen + "원)</li>");
			$(".teen_sel_sit").append("<li class='teen_sit_no teen_on'>0</li>");
			for (var i = 1; i < 9; i++) {
				$(".teen_sel_sit").append(
						"<li class='teen_sit_no teen_off'>" + i + "</li>");
			}
							...
							function(e) {
								...
								if (c.substring(0, 2) == "관람" || c.length == 5) { // 성인
									...
									if (c.substring(3, 4) > $(this).text()
										...
										var minus = c.substring(3, 4)
												- $(this).text();
										var sumcharge = charge.substring(0, 2)
												+ " - " + minus * 9;

									} else if (c.substring(3, 4) > $(this)
											.text()
					...
					.click(
							function(e) {
								...
								if (a.substring(0, 2) == "관람"
									...
									} else if (a.substring(12, 13) > $(this)
											.text()
											&& charge.length == 8) { // 10만원 이상 - 계산
										var minus = a.substring(12, 13)
												- $(this).text();
	...
	function next() {
		...
		$("#movie_img img").attr("src",
				"/SwingFlow/images/Movie/Reserve/tit_Seat.gif");
	...
	function SeatList(data) {
		...
		for (var i = 1; i <= row; i++) {
			...
			for (var j = 1; j <= column; j++) {
				if (j == 1) {
					table += "<td width='20' align='center'>" + rowname
							+ "</td>";
				} else if (j == 2) {
					table += "<td width='20' align='center'></td>";
				} else if (j > 2) {

					table += "<td class='sel_seat " + rowname + (j - 2)
							+ "' width='20' align='center'>" + rowname
							+ (j - 2) + "</td>";
				}
			}
			table += "</tr>";
		...
		$(".sel_seat")
				.click(
						function e() {
							...
							$(".seat_img")
									.attr("src",
											"/SwingFlow/images/Movie/Reserve/img_SeatArea_on.gif");
						});
	...
	function reserve() {
		...
		if (check == true) {
			location.href = "ReserveInsert.jsp?charge=" + charge + "&seatinfo="
					+ seatinfo + "&adult=" + adult + "&teen=" + teen
					+ "&mName=" + mName + "&tName=" + tName + "&time=" + time;
		}
	}
</script>
</head>
...
<body>
	...
	<jsp:include page="/Category/Common/top.jsp">
		<jsp:param value="reserve" name="gubun"/>
	</jsp:include>
	<div id="wrapper">
		...
		<div id="main_content">
			...
			<div id="sel_time">
				<div
					style="height: 265px; text-align: center; display: table-cell; width: 200px; vertical-align: middle;">
			...
			<div class="sel_agebox"
				style="float: left; margin-top: 2px; margin-right: 30px;">

				<div
					style="width: 419px; height: 83px; display: table-cell; text-align: center; vertical-align: middle;">
					<img src="http://movie-img.yes24.com/reserve/Nstep_PersonNum.gif">
				</div>
			</div>
			<div class="sel_age">
				<!-- 선택하신 영화는 '전체 관람가' 영화입니다. -->
			</div>
			</section>
			<div id="sel_sit_box">
				<center>
	...
	<%} %> --%>
					...
					<div class="sel_sit_button">
						<input type="button" value="다시선택" onclick="resel();"> <input
							type="button" value="선택완료" onclick="reserve();">
					</div>
				</center>
			</div>
